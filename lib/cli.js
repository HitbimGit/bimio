...
program
  ...
  .action(() => {
    ...
    // Get server api list configuration
    let apiList = require("./server/config.js");

    // Show api list
    for (let serverApi in apiList) {
      // Iterate over each server api
      for (let api in apiList[serverApi]) {
        // Iterate over each API in the serverApi
...
program
  .command("docs")
  .description("Show CLI docs of Hitbim Services.")
  .action(async () => {
    clearConsole();
    // Hitbim Docs page
...
program
  ...
  .action(async options => {
    ...
    if (!options.email || !options.password) {
      ...
      password =
        password ||
        readlineSync.question("password : ", {
          hideEchoBack: true,
        });
    }
    comm.log("");
