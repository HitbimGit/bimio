...
function getLocalPlugins() {
  ...
  if (_plugins.length === 0) {
    comm.guideCommand(
      "There is no plugin yet ...\nYou can create a sample plugin with following command : ",
...
function getLocalComponents() {
  ...
  const componentPath = path.join(projectPath, "public", "COMPONENTS");
  if (!fs.existsSync(componentPath)) {
    comm.logWithColor(chalk.red, "Failed to read components path ...\n");
    return null;
  }

  let _components = {};

  // Get tabbar directories
  const _tabbars = getLocalTabbars(componentPath);

  if (_tabbars !== null && _tabbars.length > 0)
    _components["tabbar"] = _tabbars;

  return _components;
}

/**
 * @function getLocalTabbars
 * @author HyunHo
 * @since 23.08.08
 */
function getLocalTabbars(componentPath) {
  const tabbarPath = path.join(componentPath, "tabbar");
  if (!fs.existsSync(tabbarPath)) {
    comm.logWithColor(chalk.dim, "COMPONENTS/tabbar does not exist ...\n");
    return null;
  }
  ...
  const directories = fs
    .readdirSync(tabbarPath, { withFileTypes: true })
    .filter(dirent => dirent.isDirectory())
    .map(dirent => dirent.name);

  for (let directory of directories) {
    _tabbars.push({ name: directory, index: "tabbar.html" });
  }
