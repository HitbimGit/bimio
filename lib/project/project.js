...
function showCmdDesc() {
  console.log();
  console.log(chalk.cyan(`  bimio run`));
  console.log("    Runs the development server.");
  console.log();
  console.log(chalk.cyan(`  bimio create -`));
  console.log("    creates a sample structure.");
  console.log();
}
...
function getLocalPlugins() {
  ...
  if (!isProjectDir(projectPath)) {
    console.log(chalk.red("Failed to read project path\n"));
    return null;
  }
  const pluginPath = path.join(projectPath, "public", "PLUGINS");
  if (!fs.existsSync(pluginPath)) {
    console.log(chalk.red("Failed to read plugin path\n"));
    return null;
  }
...
const project = {
  ...
  runProject: async userPort => {
    ...
    checkBrowsers(projectPath, isInteractive)
      .then(() => {
        console.log(chalk.cyan("Choosing port ..."));
        return choosePort(targethost, userPort);
      })
      .then(port => {
        if (port == null) {
          console.log("runnable port not found ...");
          return;
        }
        console.log(`port ${chalk.yellow(port)} was chosen ...\n`);
      ...
      .catch(err => {
        console.error("Error occured while choosing Port ...", err.message);
      });
  },
