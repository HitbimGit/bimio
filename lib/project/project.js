...
const project = {
  createProject: async projectName => {
    ...
    fs.mkdirSync(path.join(projectPath, "PLUGINS"));
    fs.mkdirSync(path.join(projectPath, "SDK"));
...
`;
    ...
    fs.writeFileSync(path.join(projectPath, "app.html"), htmlContent, "utf8");
    ...
    await plugin.createPlugin("Sample1", `${projectName}/PLUGINS`);
  },
  runProject: async => {
  },
