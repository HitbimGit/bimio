...
const logError = message => {
  comm.logWithColor(chalk.yellowBright, message);
};
 ...
 */
const formatResultMessage = (error = true, message = "", data = {}) => {
...
const checkPluginPath = (pluginName, projectPath) => {
  ...
  if (!fs.existsSync(pluginsPath)) {
    ...
    return formatResultMessage(true, msg);
  }
  ...
  return formatResultMessage(false, "Success", { path: pluginPath });
};
...
exports.createPlugin = async (pluginName, projectPath) => {
  ...
  if (checkPluginPathResult.error) {
    return logError(checkPluginPathResult.message);
  }
