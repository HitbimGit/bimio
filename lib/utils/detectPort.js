...
module.exports = (port, host, callback) => {
  ...
  console.log("detect free port between (%s, %s)", port, maxPort);
...
function listen(port, hostname, callback) {
  ...
  server.on("error", err => {
    console.log("listen %s:%s error: %s", hostname, port, err);
    server.close();
    if (err.code === "ENOTFOUND") {
      console.log("ignore dns ENOTFOUND error, get free %s:%s", hostname, port);
  ...
  server.listen(port, hostname, () => {
    ...
    server.close();
    console.log("get free %s:%s", hostname, port);
    return callback(null, port);
  });
}
